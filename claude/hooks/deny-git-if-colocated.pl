...
exit 0 unless $command =~ /(?:^|[;&|(`]|\s)git\s+(?:add|rm|commit|checkout|reset|stash|restore|clean|merge|rebase|cherry-pick|revert|am|apply|update-ref|branch|tag)(?:[;&|)`\s]|$)/m;
...
print '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny",'
    ...
    . 'authoritative. Use the jj equivalent instead of a mutating git command '
    . '(add/rm/commit/checkout/reset/stash/restore/clean/merge/rebase/cherry-pick/'
    . 'revert/am/apply/update-ref/branch/tag)."}}'
    . "\n";
